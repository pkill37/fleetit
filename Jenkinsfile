node {
    ...
    stage('Test images') {
        ...
        sh 'pwd && ls && docker run --network fleetit_network -w /app -v ./api:/app maven:3.5-jdk-9-slim pwd && ls && ls / && ls /app'

        sh 'docker run --network fleetit_network -w /app -v ./client:/app node:alpine npm install && npm test'
    }
