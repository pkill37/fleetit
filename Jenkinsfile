node {
    ...
    stage('Test images') {
        ...
        sh 'docker run -it --network fleetit_network -w /app -v $PWD/client:/app node:alpine npm install && npm test'
        sh 'docker run -it --network fleetit_network -w /app -v $PWD/api:/app maven:3.5-jdk-9-slim mvn test'
    }
